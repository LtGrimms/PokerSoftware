...
hand.prototype.parseWinnings = function(lines, endOfPlayerIndex, endOfActionIndex) {
    for (var line = endOfActionIndex; line < lines.length; line++) {
        ...
        if (pot_winnings_match) {
            this.addWinningsNoSidepot(pot_winnings_match[1], parseInt(pot_winnings_match[2],10));
        } else if (show_down_pot_match) {
            this.addWinningsNoSidepot(show_down_pot_match[1], parseInt(show_down_pot_match[2], 10));
        } else if (show_down_sidepot_match) {
            this.addWinningsSidepot(show_down_sidepot_match[1], parseInt(show_down_sidepot_match[2], 10));
        }
    }
};
