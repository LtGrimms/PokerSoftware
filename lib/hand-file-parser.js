...
function parse_hand(data) {
    ...
    var yourHoleCards = {};
    var yourHoleCardsRE = /Dealt to (.+) \[(.{2}) (.{2})]/; // you escaped the [ but not the ]
    ...
    var buttonSeat = parseInt(data.match(buttonPositionRE)[1]); // parse int has a second parameter which you should always use.
    // So this should be parseInd(data.match(buttonPositionRE)[1], 10);
    ...
    while (lines[line].match(/Seat \d: .* \(\d* in chips\)/)) {
        ...
        var playerSeat = parseInt(lines[line].match(seatRE)[1]); // Once again, add the radix
        var seat = {
            handNumber: hand.number,
            name: name,
            chips: chips,
