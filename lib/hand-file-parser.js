...
//Parse The Hand Information
    ...
    if (!tournament) {
        ...
        timestamp: new Date(data.match(reLib.handTimeStamp)[1]),
        tournament: data.match(reLib.handTournamentNumber)[1],
        ante: null,
        smallBlind: parseInt(data.match(reLib.handSmallBlindAmount)[1], 10),
        bigBlind: parseInt(data.match(reLib.handBigBlindAmount)[1], 10),
...
function parseActions(lines, endOfPlayerIndex, endOfActionIndex, handNumber, bigBlindAmount) {
    ...
    for (var i = endOfPlayerIndex; i < endOfActionIndex; i++) {
        ...
        if (lines[i].match(reLib.postAnte)) {
            ...
            action = createAction(handNumber, name, round, "post ante", amount, 0, actionNumber);
            actionNumber++;
            actions.push(action);
            ...
            var name = match[1];
            var amount = parseInt(match[2]);
            action = createAction(handNumber, name, round, "post blind", amount, 0, actionNumber);
            actionNumber++;
            actions.push(action);
        } else if (lines[i].match(reLib.fold)) {
            var match = lines[i].match(reLib.fold);
            var name = match[1];
            action = createAction(handNumber, name, round, "fold", null, currentBetSize, actionNumber);
            actionNumber++;
            actions.push(action);
            ...
            var name = match[1];
            var amount = parseInt(match[2]);
            action = createAction(handNumber, name, round, "call", amount, currentBetSize, actionNumber);
            actionNumber++;
            actions.push(action);
            ...
            action = createAction(handNumber, name, round, "bet", betSize, currentBetSize, actionNumber);
            actionNumber;
            actions.push(action);
            currentBetSize = betSize;
        } else if (lines[i].match(reLib.betAllIn)) {
            var match = lines[i].match(reLib.betAllIn);
            var name = match[1];
            var betSize = parseInt(match[2]);
            action = createAction(handNumber, name, round, "bet all-in", betSize, currentBetSize, actionNumber);
            actionNumber++;
            actions.push(action);
            currentBetSize = betSize;
            ...
            action = createAction(handNumber, name, round, "raise", raiseAmount, currentBetSize, actionNumber);
            actionNumber++;
            actions.push(action);
            ...
            var name = match[1];
            var raiseAmount = parseInt(match[2]);
            action = createAction(handNumber, name, round, "raise all-in", raiseAmount, currentBetSize, actionNumber);
            actionNumber++;
            actions.push(action);
            currentBetSize += raiseAmount;
